...
<main class="flex flex-row min-h-full h-full min-w-full justify-between bg-base-100">
    ...
    <Grid bind:items={items}  rowHeight={100} let:item {cols} let:index fastStart={true} fillSpace={true} gap={[5,0]} >
      <div class="flex bg-base-300 h-full w-full overflow-y-auto no-scrollbar">
        <table class="table-auto ">
          <thead>
            <tr>
              <th class="text-base-content text-sm">{bybit_book.full_market_name}</th>
            </tr>
          </thead>
